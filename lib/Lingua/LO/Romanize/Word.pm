...
sub _find_lao_syllable {
    ...
    # ຫ foljt av ຍ gn, ນ n, ມ m, ຣ r, ລ l, or ວ v. ຫ skall inte ha vokaler eller toner
    # alternativt K foljt av ຣ r,  ລ (ຼ) l,  or  ວ o . ແຂວງ → Khoèng ຂວ==K 
    
    if ($consonant eq 'ຫ' && $word =~ /^ຫ[ເ-ໄ]?([ນມຍຣລວຼ])/) {
        $found_ha = $consonant;
        $consonant = $1;
    } elsif ($consonant =~ /[^ຣລຼວ]/ && $word =~ /^[ເໂແ]?$consonant([ຣລຼວ])/) {
        my $extra = $1;
        if ($word =~ /^([ເໂແ]?)$consonant$extra([ະັາິີຶືຸູະັົອໍວຽຍຳ່້໊໋]*)/ && ($1 || $2)) {
            $consonant .= $extra;
        }
    }
    # build regexp depending on ha or not
    my $regexp = qr{$found_ha([ເ-ໄ])?$consonant([ະັາິີຶືຸູະັົອໍວຽຍຳ່້໊໋]*)};
    ...
    if ($vowels =~ s/([່-໋])//) {
        $tone = $1;
        $regexp = qr{$found_ha(?:[ເ-ໄ]?)$consonant(?:[ັິີຶືຸູັົໍ]?)$tone};
        $tone = undef unless $word =~ /^$regexp/;
    }
    ...
    if ($vowels =~ /^(?:ໍາ|ຳ)/) { #'sala am' is always the end of a syllable
        ...
        if ($found =~ /^([ເ-ໄ])$/) {
            ...
            $syllable .= $tone if defined ($tone);
            return $found_ha.$syllable if $found =~ /[ໄໃ]/; #ໄໃ should be the final
    ...
    $regexp = qr{$found_ha$syllable([ກງຍຽດນບມຣວ]໌?)(?:([^່້໊໋ະັາິີຶືຸູະັົໍຳ])(.?)|$)};
